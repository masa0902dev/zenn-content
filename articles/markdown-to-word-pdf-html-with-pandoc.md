...
emoji: "✍️"
type: "tech"
topics: ["Markdown", "Word", "PDF", "pandoc", "zsh"]
published: true
---
...
事務仕事の場合でも, WordからMarkdownに乗り換えるメリットはあります!
Markdown はプレーンテキストなので, 10年後も同じファイルをどんなエディタでも開けます. Word ファイルはソフトウェアのバージョン・ライセンス等に大いに依存しますが, Markdown はその心配がありません. 長期的に文書を保管・管理する観点でも, Markdown は優れた選択肢です.
...
zsh は macOS のデフォルトシェル (ターミナルで使われるコマンド実行環境) であり, `~/.zshrc` はそのシェルが起動するたびに自動で読み込まれる設定ファイルです. (bashの場合は `~/.bashrc` で読み替えてください.)

`~/.zshrc` に以下を追記してください. ファイルが無い場合は作成してください.
VSCodeでは`code ~/.zshrc`でエディタ内にファイルを開けます.

以下の5つの関数を定義します. `md2docx`・`md2html`・`md2pdf` が各形式への変換を, `md2all` がまとめて変換を, `md2rmall` が生成ファイルの削除を担当します.
...
すべての変換関数は同じ引数インターフェースを持ちます.

- **第1引数** (必須): 入力 Markdown ファイルのパス
- **第2引数** (省略可): 出力ファイルのベース名 (拡張子なし・拡張子付きで渡しても自動除去. 省略時は入力ファイル名を使用)
...
**`pandoc: command not found` が出る**
Pandoc がインストールされていません. `brew install pandoc` でインストールしてください.

**`❌ md2pdf:failed` が表示される**
以下の順に確認してください.
1. pandoc 単体での HTML 変換が動くか: `pandoc input.md -o output.html --standalone --mathjax`
2. Chrome のパスが正しいか: `ls "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"` でファイルの存在を確認
3. `source ~/.zshrc` を実行して関数を再読み込みしたか
...
また, Markdown での執筆に慣れると, 文書作成スピード・快適さが上がります. Markdown で書いた文書はプレーンテキストですから, Word よりも柔軟にテキスト検索もでき, Git で完全な変更履歴を残すことも容易です. この記事が Markdown への移行のきっかけになれば幸いです.
