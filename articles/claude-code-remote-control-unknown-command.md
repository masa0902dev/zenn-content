...
一見すると「自分のClaude Codeのバージョンが古くてRemote Control機能自体が存在しない」ように見えますが, 実際には最新版のClaude Codeでも, 設定次第でこのエラーが発生します. (npmインストールではなくnativeインストールしていても同様です)
...
検証に使用した環境は次の通りです. バージョンやOSが異なると, 表示されるエラーメッセージや対象となる環境変数が変わる可能性があるため, あくまで参考情報として確認してください.
