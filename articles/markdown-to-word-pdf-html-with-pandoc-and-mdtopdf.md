...
emoji: "✍️"
type: "tech"
topics: ["Markdown", "Word", "PDF", "pandoc", "mdtopdf"]
published: false
---
...
本記事では, **Pandoc** と **md-to-pdf** を使って Markdown を Word・PDF・HTML に変換するコマンドを整備します. さらに, どのディレクトリからでも呼び出せるよう zsh の関数として登録します.
...
- **軽量で高速**
  - 大きなファイルでもサクサク編集できます.
- **変換が容易**
  - HTML, Word, PDF, LaTeX, ODT, など様々な形式に変換できます.
...
Word ↓
![Wordへの変換結果](/images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/appearance-word1.png)
![Wordへの変換結果](/images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/appearance-word2.png)

PDF ↓
![PDFへの変換結果](/images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/appearance-pdf1.png)
![PDFへの変換結果](/images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/appearance-pdf2.png)

HTML ↓
![HTMLへの変換結果](/images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/appearance-html1.png)
![HTMLへの変換結果](/images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/appearance-html2.png)
...
![サンプル画像: 愛犬](images/markdown-to-word-pdf-html-with-pandoc-and-mdtopdf/bolt_512.jpeg){width=150px}
...
- OS: macOS (本記事のコマンドは macOS 向けです)
- シェル: zsh
- 必須アプリ: Google Chrome (PDF 変換に使用)
...
`--standalone` で `<html>`, `<head>`, `<body>` を含む完全な HTML ファイルが出力されます. `--mathjax` で MathJax の CDN スクリプトが埋め込まれ, ブラウザで数式が正しく表示されます.

## PDF (.pdf)

PDF は HTML を経由して生成します.

```bash
# Step 1: Markdown → HTML (with MathJax)
pandoc input.md -o output.html --standalone --mathjax
...
Chrome のヘッドレスモードは MathJax を含む JavaScript を実行した上で PDF を生成するため, 数式も正しく出力されます. `--virtual-time-budget=10000` は「ページ内の JavaScript (MathJax のレンダリング) が完了するまで最大10秒待ってから印刷する」という指定です. これがないと数式の描画が終わる前に PDF 化されてしまいます.

---

# どこからでも使えるようにする

毎回上記のコマンドを打つのは面倒です. **zsh の関数として登録**することで, どのディレクトリからでも短いコマンドで呼び出せるようにします.

:::message
zsh は macOS のデフォルトシェル (ターミナルで使われるコマンド実行環境) です. `~/.zshrc` はそのシェルが起動するたびに自動で読み込まれる設定ファイルです.
:::
...
:::details コードの詳細説明
- `local dir="$(dirname "$1")"`: 入力ファイルのディレクトリパスを取得します. 例: `path/to/file.md` → `path/to`
- `${$(dirname "$1"):A}`: `md2pdf` では Chrome に渡す `file://` URL に絶対パスが必要なため, zsh の `:A` 修飾子で絶対パスを取得しています. `cd` を使う方法では `chpwd` フックが発火するため, この方式を採用しています.
- `${2:-$(basename "${1%.*}")}`: 第二引数があればそれを, なければ入力ファイル名から拡張子を除いたものをベース名として使います.
- `html_tmp`: `md2pdf` 内で一時的に生成する HTML ファイルです. PDF 変換後に自動で削除されます.
- `> /dev/null 2>&1`: コマンドの出力を全て捨てます (ログを表示しないため).
- `local`: 変数のスコープを関数内に限定します. 関数の外にある同名の変数を上書きしません.
:::
...
`source ~/.zshrc` は, 設定ファイルを現在のターミナルに再読み込みさせるコマンドです. 新しいターミナルウィンドウを開けば自動で読み込まれるため, その場合は不要です.
...
md-to-pdf は `--stylesheet` オプションで CSS を適用できます.

```bash
md-to-pdf input.md --stylesheet my-style.css
```
