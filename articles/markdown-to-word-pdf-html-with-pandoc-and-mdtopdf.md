...
- `${2:-$(basename "${1%.*}")}`
  第二引数があればそれを, なければ入力ファイル名から拡張子を除いたものをベース名として使います.
- `> /dev/null 2>&1`
...
`md-to-pdf` を使えば `--stylesheet` オプションで CSS を適用できます.
...
Markdown で書いても Wordや PDF に出力できるようになれば, 「提出が Word だから…」という理由で Markdown から離れる必要はなくなります.
