...
|          | 手順                                                                                                                                                                                          |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 今まで   | 1. Raycastで"VPN"と打ち込み, System SettingsのVPN画面を開く<br>2. 対象VPNの接続ボタンをクリック<br>3. iPhoneを開く<br>4. iPhoneの認証アプリを開く<br>5. 認証アプリのポップアップでYesをタップ |
| 効率化後 | 1. Raycastで `cmd+5`<br>2. iPhoneのロック画面で認証用ショートカットをタップ<br>3. 認証アプリのポップアップでYesをタップ                                                                       |
...
iPhone:
- iPhone側で"2段階認証アプリ"を設定済みであること
(本記事では"認証アプリ"と呼びます. Microsoft Authenticatorなど, お使いのアプリに読み替えてください)

今回利用するRaycastをまだ使ったことがない方は, 下記リンク等を参考にインストール・利用してみて下さい. Finderをめちゃくちゃ便利かつ多機能にしたもの, というイメージです (いずれ記事で著者のユースケースを紹介したいですね)
...
- macOS側
  1. VPNの名称をシンプルなものに変更する(日本語は避ける)
  2. homebrewで`vpnutil`をインストールし, VPNの起動/停止ができることを確認する
  3. Raycast Script Commandとして, VPNの起動用/停止用コマンドを作成する
  4. 作成したコマンドをRaycastのFavoriteに登録し, ショートカットキーを割り当てる
- iPhone側
  1. Shortcutsアプリで「認証アプリを開くだけ」のショートカットを作成する
  2. そのショートカットをロック画面に配置する
