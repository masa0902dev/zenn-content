...
`--json` オプションはAPIの生レスポンスをそのまま出力します. ツールをカスタマイズする際や, レスポンスの構造を確認したいときに使えます.
...
`clc rate` 一発でClaude Codeのレート残量を確認できるようにしました.

コードはシンプルなので, 表示をカスタマイズしたり別のサブコマンドを追加したりしやすいと思います. macOS, 使って頂けると嬉しいです.
